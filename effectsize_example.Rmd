...
▸ Exemplar: [simple effect size](#simple-effect-size) and [standardized effect size](#standardized-effect-size)
...
▸ Exemplar: [standardized effect size](#standardized-effect-size)
...
▸ Exemplar: [Nonparametric effect size](#nonparametric-effect-size)
...
library(broom) # for extracting model results in table format
o_ <- function(x, k = 2) format(round(x, k), nsmall = k)  # for decimal point formatting
```

## Data

Scenario: An experiment comparing two group of users in terms of their task completion time (between-subjects design).
```{r}
set.seed(12)
n <- 20
data <- tibble(
...
This data can be visualized as follow:
...
*Simple effect size:* The task completion time of the two groups differ by `r o_(t_result$estimate)` (95% confidence interval [`r o_(t_result$conf.low)`, `r o_(t_result$conf.high)`]) seconds.
...
**Standardized effect size:** Cohen's d = `r o_(cohen_d$estimate)` SDs with 95% confidence interval [`r o_(cohen_d$conf.int["inf"])` , `r o_(cohen_d$conf.int["sup"])`]
...
**Non-parametric effect size:** Variance-based effect size *r*  = `r o_(effect_r)`.
